...
def openDataWindow():
    ...
    command1 = obd.commands.COOLANT_TEMP
    command2 = obd.commands.RPM
    command3 = obd.commands.THROTTLE_POS

    def coolantTemperatureTracker(clt_t):
        global coolantTemperature
        if not clt_t.is_null():
            coolantTemperature = int(s.value.magnitude)

    def rpmTracker(rpm_t):
        global rpm
        if not rpm_t.is_null():
            rpm = int(s.value.magnitude)

    def throttlePositionTracker(tp_t):
        global throttlePosition
        if not tp_t.is_null():
            throttlePosition = int(s.value.magnitude)

    #Watches the data extracted by the obd adapter
    connection.watch(command1, callback=coolantTemperatureTracker)
    connection.watch(command2, callback=rpmTracker)
    connection.watch(command3, callback=throttlePositionTracker)
    connection.start()
